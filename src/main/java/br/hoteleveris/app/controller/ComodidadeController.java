 ...
 * @RestController
 * 
 * @RequestMapping("/comodidades") public class ComodidadeController extends
 * BaseController{
 * 
 * private final ComodidadeService _service;
 * 
 * @Autowired public ComodidadeController(ComodidadeService service) { _service
 * = service; }
 * 
 * @PostMapping public ResponseEntity inserir(@RequestBody ComodidadeRequest
 * comodidadeRequest) { try { BaseResponse response =
 * _service.inserir(comodidadeRequest); return
 ...
 * id) { try { ComodidadeResponse response = _service.obter(id); return
 * ResponseEntity.status(response.StatusCode).body(response); } catch (Exception
 * e) { return ResponseEntity.status(errorBase.StatusCode).body(errorBase); } }
 * 
 * @GetMapping public ResponseEntity listar() { try { ComodidadeList comodidades
 * = _service.listar(); return
 * ResponseEntity.status(HttpStatus.OK).body(comodidades); } catch (Exception e)
 ...
 * ComodidadeRequest comodidadeRequest, @PathVariable Long id) { try {
 * BaseResponse response = _service.atualizar(id, comodidadeRequest); return
 * ResponseEntity.status(response.StatusCode).body(response); } catch (Exception
 * e) { return ResponseEntity.status(errorBase.StatusCode).body(errorBase); } }
 * 
 * }

...
public class TipoQuartoController extends BaseController{
	...
	public ResponseEntity inserir(@RequestBody TipoQuartoRequest tipoQuartoRequest) {
		try {
			BaseResponse response = _service.inserir(tipoQuartoRequest);
	...
	@PutMapping(path = "/{id}")
	public ResponseEntity atualizar(@RequestBody TipoQuartoRequest tipoQuartoRequest, @PathVariable Long id) {
		try {
			BaseResponse response = _service.atualizar(id, tipoQuartoRequest);
			return ResponseEntity.status(response.StatusCode).body(response);
		} catch (Exception e) {
			return ResponseEntity.status(errorBase.StatusCode).body(errorBase);
		}
